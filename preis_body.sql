...
PROCEDURE export_ausschreibung_to_excel_gaeb90(
    ...
    p_regionen varchar2, p_liferant varchar2, p_user_id number
       ...
       -- INLINE PROZEDUR: Position in Datenbank wegschreiben
       ...
       BEGIN
           -- GAEB 90 Positionsnummer (z.B. 01010010) in Hierarchie aufdröseln
           l_master_code := SUBSTR(v_pos_code, 1, 2);
           l_parent_code := SUBSTR(v_pos_code, 1, 4);
           l_item_code   := SUBSTR(v_pos_code, 5, 4);

           -- Hierarchie-Namen aus dem Status-Speicher holen
           ...
           -- XML-Kompatible Formatierung erzwingen (z.B. 01.0101.0010)
           l_master_fmt := l_master_code;
           l_parent_fmt := l_master_code || '.' || l_parent_code;
           l_code_fmt   := l_parent_fmt || '.' || l_item_code;

           -- MLV-Kennung exakt wie in _unified aus dem Text extrahieren
           ...
           -- Preise in Datenbank nachschlagen
           BEGIN
           ...
           VALUES
                (p_blob_id, l_id, v_pos_name, l_code_fmt, v_pos_me, v_pos_menge, l_min, l_avg, l_max, l_median, l_trimm, l_parent_fmt, l_master_fmt, l_parent_name, l_master_name, v_pos_kennung, l_count, 0, null, null);
...
BEGIN
    ...
    -- =========================================================================
    -- SCHRITT 1: EXCEL SETUP (Identisch zu _unified)
    -- =========================================================================
    ...
    -- =========================================================================
    -- SCHRITT 2: GAEB 90 PARSER (BLOB zu CLOB und Zeile für Zeile verarbeiten)
    -- =========================================================================
    ...
    LOOP
        ...
        -- Aktuelle Zeile lesen und carriage returns entfernen
        ...
        IF SUBSTR(v_line, 1, 2) = '11' THEN
            -- Hierarchie Code merken (z.B. 01 oder 0101)
            v_current_lvl := TRIM(SUBSTR(v_line, 3, 9));
        ELSIF SUBSTR(v_line, 1, 2) = '12' THEN
            -- Hierarchie Name zum letzten Code in Map speichern
            IF v_current_lvl IS NOT NULL THEN
                v_hierarchies(v_current_lvl) := TRIM(SUBSTR(v_line, 3, 72));
            END IF;
        ELSIF SUBSTR(v_line, 1, 2) = '21' THEN
            -- Wenn wir bei einer neuen Position ankommen, erst die alte wegschreiben!
            IF v_pos_code IS NOT NULL THEN
                insert_parsed_position;
            END IF;

            -- Neue Positionsdaten auslesen
            v_pos_code := TRIM(SUBSTR(v_line, 3, 9));
            BEGIN
                -- In GAEB 90 ist die Menge auf 11 Zeichen formatiert mit impliziten 3 Nachkommastellen (z.B. 00000001000 = 1.000)
        ...
        END IF;
        -- Anmerkung: Zeilentyp 26 (Langtext) wird für diesen Export (vorerst) nicht benötigt.
    END LOOP;

    -- Am Ende der Datei die allerletzte Position wegschreiben
    IF v_pos_code IS NOT NULL THEN
        insert_parsed_position;
    END IF;
    ...
    -- =========================================================================
    -- PRÜFUNG & HEADER (NACHDEM DIE DATEN IN DIE TABELLE GESCHRIEBEN WURDEN)
    -- =========================================================================
    v_has_menge := false;
    begin
    ...
    xlsx_writer.col_width(workbook, sheet_1, l_col, 10); l_col := l_col + 1; -- MENGE (now on D)
    xlsx_writer.col_width(workbook, sheet_1, l_col, 8);  l_col := l_col + 1; -- EINHEIT (now on E)
    xlsx_writer.col_width(workbook, sheet_1, l_col, 20); l_col := l_col + 1; -- VAL. PREIS
    ...
    -- =========================================================================
    -- SCHRITT 3: EXCEL GENERIERUNG (Identisch zu _unified)
    -- =========================================================================
    ...
    IF l_row > 2 THEN
         xlsx_writer.add_data_validation(
            ...
            sheet => sheet_1, 
            sqref => 'F2:F' || (l_row - 1), 
            formula1 => '"Mittelwert,Median,getrimmter MW"'
        );
    END IF;
...
EXCEPTION 
    WHEN others THEN
        ...
        -- Triggert E-Mail auch beim Absturz dieser speziellen Routine
    ...
    if l_mustern.count > 0 then
        for idx in 1..l_mustern.count loop
            ...
            if l_mustern(idx).parent_master = 'PARENT' then
                ...
                l_col := l_col + 1;
                IF p_trimm IS NOT NULL THEN
                    if l_mustern(idx).getrimmter_preis is not null then xlsx_writer.add_cell(workbook, sheet_1, l_row_index, l_col, style_id => number_format, value_ => l_mustern(idx).getrimmter_preis); end if;
                    l_col := l_col + 1;
                END IF;
